...
`describe()`는 기본적으로 숫자형 변수의 요약 통계량을 보여줌
...
- 여기서 `total`은 학습을 위한 단순 평균
- 실제 공인 복합 연비 계산식과 같다고 단정하면 안 됨
...
- 메서드 체이닝은 중간 변수를 줄일 수 있음
- 다만 코드가 너무 길어지면 여러 단계로 나누는 편이 디버깅하기 쉬움

---
# 14. 얕은 복사와 깊은 복사

Python의 변수에는 객체 자체가 들어가는 것이 아니라, **객체가 저장된 메모리 위치를 가리키는 참조**가 저장됨
...
`a`와 `b`의 목적지가 같기 때문에 어느 쪽을 수정해도 같은 객체가 변경됨
...
데이터 분석에서는 원본 데이터프레임을 보존하기 위해 다음과 같이 복사본을 만듦

```python
mpg_new = mpg.copy()
```

이후 `mpg_new`의 열 이름을 바꿔도 일반적인 데이터프레임 작업에서는 원본 `mpg`가 유지됨
...
다만 데이터프레임의 셀 안에 리스트 같은 **변경 가능한 Python 객체가 들어 있으면**, 그 객체는 재귀적 복사 대상에서 제외
...
평균값을 `0.4872`처럼 직접 코드에 입력하지 않는 것이 중요
...
4. 원본 보존이 필요하면 `copy()`로 복사본을 만듦
5. 변수명은 `rename(columns={...})`으로 변경함
6. 새 열은 `df["new"] = 계산식`으로 만듦
