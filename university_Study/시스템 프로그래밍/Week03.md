...
**Shell**: 사용자와 운영체제 사이에서 명령어를 전달하고 처리하는 소프트웨어

- 사용자가 명령어를 입력하면 Shell이 이를 해석하여 적절한 프로그램을 실행함
- 따라서 Shell은 **명령어 처리기(Command Processor)**라고도 함
...
변경된 로그인 Shell은 일반적으로 로그아웃 후 다시 로그인해야 적용됨

### 서브 Shell 실행

현재 Shell에서 다른 Shell을 실행하면 서브 Shell이 생성됨
...
Shell 변수는 지역변수와 환경변수로 구분됨

- 지역변수: 현재 Shell에서만 사용됨
- 환경변수: 자식 프로세스와 서브 Shell에 전달됨
...
환경변수, 프롬프트, 명령어 경로, 별명, 함수 등을 설정하는 데 사용됨
...
- 명령어를 전면에서 실행함
- 명령어 실행이 끝날 때까지 Shell은 다음 명령을 기다림
...
백틱 문법도 사용할 수 있지만 중첩과 가독성 측면에서 `$(...)` 사용이 권장됨
...
- 지역변수는 현재 Shell에서만 사용되고 환경변수는 자식 프로세스에 전달됨
