...
|---|---|---|---|
| 분할정복 | 큰 문제를 작은 문제로 분할 | 분할은 하향식, 해결·결합은 상향식 | 병합 정렬, 이진 탐색 |
| 그리디 | 현재 상태에서 가장 좋은 선택 | 앞에서부터 선택을 확정 | Kruskal, 거스름돈 |
| 백트래킹 | 선택을 시도하고 막히면 복귀 | 깊이 탐색 후 이전 상태로 복귀 | 미로 찾기, N-Queen |
...
3. **결합(Combine)**: 부분 문제의 결과를 합쳐 원래 문제의 답을 만듦

- 분할 과정은 큰 문제에서 작은 문제로 내려가는 하향식(top-down)이고, 결과를 결합하는 과정은 작은 문제에서 큰 문제로 올라가는 상향식(bottom-up)임
- 같은 구조가 반복되므로 재귀 함수와 함께 자주 사용됨
...
이 방식에서는 같은 점수를 받은 학생이 같은 등수가 됨
...
- 그리디 알고리즘의 선택은 일반적으로 나중에 취소하지 않음
- 따라서 현재의 최선 선택이 전체 문제의 최적해로 이어진다는 성질이 있는 문제에서 사용해야 함
...
- 이 가운데 선택한 간선의 가중치 합이 가장 작은 신장 트리를 최소 비용 신장 트리라고 함

최소 비용 신장 트리는 다음과 같은 문제에 활용됨
...
- 이 동전 체계에서는 그리디 방식으로 최소 동전 수를 구할 수 있음
- 임의의 동전 체계에서는 그리디 방식이 항상 최적해를 보장하지 않음
  - 예시: 동전이 1원, 3원, 4원이고 금액이 6원일 때 그리디 결과는 `4 + 1 + 1`, 최적해는 `3 + 3`
...
- 처음 `(0, 0)`에서 오른쪽 `(0, 1)`로 이동하면 막다른 길
- `(0, 1)`을 스택에서 꺼내고 `(0, 0)`으로 돌아간 뒤 아래쪽을 탐색함
